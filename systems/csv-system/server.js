...
class PKCSVParser {
  ...
  parseQuotation(lines) {
    // Similar to parseSalesOrder but for quotations
    const result = {
...
app.post('/upload', upload.single('csvFile'), async (req, res) => {
  try {
    ...
    console.log('Document Type:', parseResult.documentType);
...
app.listen(PORT, () => {
  ...
  console.log(`💾 Database: PostgreSQL on port ${process.env.DB_PORT || 5434}`);
...
module.exports = app;
EOF# สร้างไฟล์ server.js สำหรับ CSV
cat > server.js << 'EOF'
// PK CRM - Enhanced Server.js for CSV Processing
require('dotenv').config();
const express = require('express');
